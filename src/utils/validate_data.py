...
def validate_telco_data(df) -> Tuple[bool, List[str]]:
    ...
    """
    print("[Validation] Starting data validation with Great Expectations...")
    
    # Convert pandas DataFrame to Great Expectations Dataset
    import pandas as pd
    df_temp = df.copy()
    df_temp = df_temp.rename(columns={"gender": "Gender", "tenure": "Tenure"})
    if "TotalCharges" in df_temp.columns:
        df_temp["TotalCharges"] = pd.to_numeric(df_temp["TotalCharges"], errors="coerce")
    ge_df = ge.dataset.PandasDataset(df_temp)
    
    # === SCHEMA VALIDATION - ESSENTIAL COLUMNS ===
    print("   [Schema] Validating schema and required columns...")
    ...
    print("   [Business Logic] Validating business logic constraints...")
    ...
    print("   [Numeric Ranges] Validating numeric ranges and business constraints...")
    ...
    print("   [Statistics] Validating statistical properties...")
    ...
    print("   [Consistency] Validating data consistency...")
    ...
    print("   [Validation Engine] Running complete validation suite...")
    ...
    if results["success"]:
        print(f"[OK] Data validation PASSED: {passed_checks}/{total_checks} checks successful")
    else:
        print(f"[Error] Data validation FAILED: {failed_checks}/{total_checks} checks failed")
